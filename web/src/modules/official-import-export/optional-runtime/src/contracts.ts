...
export const createImportExportFetchTransport = (options: {
  ...
  const result = async (request: Promise<{ data?: unknown; error?: unknown; response: Response }>): Promise<ImportExportTransportResult> => {
    const resolved = await request
    return { body: resolved.data ?? resolved.error ?? null, headers: resolved.response.headers, status: resolved.response.status }
  }
  return {
    list(status, page, pageSize, signal) {
      return result(sdk.listImportExportOperations({
    ...
    submitImport(providerKey, fileKey, mapping, idempotencyKey, signal) {
      return result(sdk.submitImportOperation({
        body: { provider_key: providerKey, file_key: fileKey, mapping }, credentials: 'include',
        headers: { Accept: 'application/json', 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey },
        params: { header: { 'Idempotency-Key': idempotencyKey } }, signal,
      }))
    },
    submitExport(providerKey, idempotencyKey, signal) {
      return result(sdk.submitExportOperation({
        body: { provider_key: providerKey }, credentials: 'include',
        headers: { Accept: 'application/json', 'Content-Type': 'application/json', 'Idempotency-Key': idempotencyKey },
        params: { header: { 'Idempotency-Key': idempotencyKey } }, signal,
      }))
    },
    cancel(operationKey, revision, signal) {
      return result(sdk.cancelImportExportOperation({
        body: { revision }, credentials: 'include',
        headers: { Accept: 'application/json', 'Content-Type': 'application/json' },
        params: { path: { operationKey } }, signal,
      }))
    },
    download(fileKey, signal) {
      return fetcher(new Request(new URL(`/adminapi/api/v1/files/${encodeURIComponent(fileKey)}/content`, options.baseUrl), {
        credentials: 'include', headers: { Accept: 'text/csv' }, signal,
      }))
    },
  }
}
