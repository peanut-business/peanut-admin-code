<script setup lang="ts">
  ...
  const runtime = useImportExportRuntime();
  const state = runtime.state;
  const providerKey = ref('');
  const inputFileKey = ref('');
  const mappingJson = ref('{}');
  const canCreate = computed(runtime.canCreate);
  const canCancel = computed(runtime.canCancel);
  const statuses = [
    'queued',
    'running',
    'cancel_requested',
  ...
  const submitImport = async (): Promise<void> => {
    try {
      const mapping = JSON.parse(mappingJson.value) as unknown;
      if (
        ...
        throw new Error();
      await runtime.submitImport(
        providerKey.value,
        inputFileKey.value,
        mapping as Record<string, string>
      );
    } catch {
      /* Runtime validation reports request errors; malformed JSON stays local. */
    }
  };
...
<template>
  <PageContent class="import-export-page">
    ...
    <ModuleUnavailableState
      ...
      @action="runtime.load"
    />
    <section v-else-if="state.error" role="alert">
      <h2>Unable to complete the import/export request</h2
      ><p>{{ state.error.message }}</p
      ><p v-if="state.error.requestId">
        Request ID: {{ state.error.requestId }}
      </p>
    </section>
    <div v-else-if="state.loading" role="status">
    ...
    <div v-else class="table-wrap">
      <table>
        <thead
          ><tr
            ><th>Provider</th><th>Direction</th><th>Status</th><th>Progress</th
            ><th>Rows</th><th>Expires</th><th>Actions</th></tr
          ></thead
        ><tbody>
          <tr v-for="operation in state.items" :key="operation.operationKey">
            <td>{{ operation.providerKey }}</td
            ...
            ><td>
              ...
              /> </td
            ><td
              >{{ operation.acceptedRows }} accepted /
              {{ operation.rejectedRows }} rejected</td
            ><td>{{ operation.retentionUntil }}</td
            ><td>
              <ElButton
