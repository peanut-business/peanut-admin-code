...
export const parsePage = <T>(value: unknown, parser: (item: unknown) => T): Page<T> => {
  const body = record(value); exact(body, ['data', 'meta']); const data = record(body.data); const meta = record(body.meta)
  exact(data, ['items']); exact(meta, ['request_id', 'page', 'page_size', 'total'])
  if (!Array.isArray(data.items) || !requestId(meta.request_id) || typeof meta.page !== 'number' || !Number.isSafeInteger(meta.page) || meta.page < 1
    || typeof meta.page_size !== 'number' || !Number.isSafeInteger(meta.page_size) || meta.page_size < 1 || meta.page_size > 100
    || typeof meta.total !== 'number' || !Number.isSafeInteger(meta.total) || meta.total < 0) throw new Error('INTEGRATION_RESPONSE_INVALID')
  return { items: data.items.map(parser), page: meta.page, pageSize: meta.page_size, total: meta.total }
}
const json = (value: unknown): string => JSON.stringify(value)
export const createIntegrationSecurityFetchTransport = (options: { readonly baseUrl: string; readonly authorization?: () => string | null; readonly fetch?: (request: Request) => Promise<Response> }): IntegrationSecurityTransport => {
  const fetcher = options.fetch ?? fetch
  const request = async (path: string, init: RequestInit): Promise<TransportResult> => {
    const token = options.authorization?.() ?? null
    const response = await fetcher(new Request(new URL(path, options.baseUrl), { credentials: 'same-origin', ...init, headers: { Accept: 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}), ...init.headers } }))
  ...
  return {
    machines: signal => request('/adminapi/api/v1/integration-security/machine-identities', { method: 'GET', signal }),
    createMachine: (input, signal) => write('/adminapi/api/v1/integration-security/machine-identities', 'POST', input, signal),
    rotateMachine: (key, revision, signal) => write(`/adminapi/api/v1/integration-security/machine-identities/${encodeURIComponent(key)}/rotate`, 'POST', { revision }, signal),
    revokeMachine: (key, revision, signal) => write(`/adminapi/api/v1/integration-security/machine-identities/${encodeURIComponent(key)}`, 'DELETE', { revision }, signal),
    webhooks: signal => request('/adminapi/api/v1/integration-security/webhooks', { method: 'GET', signal }),
    createWebhook: (input, signal) => write('/adminapi/api/v1/integration-security/webhooks', 'POST', input, signal),
    rotateWebhook: (key, revision, signal) => write(`/adminapi/api/v1/integration-security/webhooks/${encodeURIComponent(key)}/rotate-secret`, 'POST', { revision }, signal),
    disableWebhook: (key, revision, signal) => write(`/adminapi/api/v1/integration-security/webhooks/${encodeURIComponent(key)}`, 'DELETE', { revision }, signal),
    deliveries: (page, pageSize, signal) => request(`/adminapi/api/v1/integration-security/deliveries?page=${page}&page_size=${pageSize}`, { method: 'GET', signal }),
    deliveryAttempts: (key, page, pageSize, signal) => request(`/adminapi/api/v1/integration-security/deliveries/${encodeURIComponent(key)}/attempts?page=${page}&page_size=${pageSize}`, { method: 'GET', signal }),
    sessions: signal => request('/adminapi/api/v1/integration-security/sessions', { method: 'GET', signal }),
    revokeSession: (key, signal) => write(`/adminapi/api/v1/integration-security/sessions/${encodeURIComponent(key)}/revoke`, 'POST', {}, signal),
  }
}
