...
export interface FileAssetRuntimeOptions {
  ...
  readonly request?: (page: number, pageSize: number, signal: AbortSignal) => Promise<Response>
}

const defaultRequest = (page: number, pageSize: number, signal: AbortSignal): Promise<Response> => {
  const token = getToken()
  return fetch(`/adminapi/api/v1/files/assets?page_no=${page}&page_size=${pageSize}`, {
    method: 'GET',
...
export const createFileAssetRuntime = (options: FileAssetRuntimeOptions): FileAssetRuntime => {
  const state = reactive<FileAssetState>({
    items: [], selectedFileKey: null, page: 1, pageSize: 24, total: 0,
  ...
  const load = async (page = state.page): Promise<void> => {
    ...
    try {
      if (!options.canRead()) {
        state.error = { message: '你没有查看图片素材的权限。', requestId: null, status: 403 }
        return
      }
      const response = await (options.request ?? defaultRequest)(page, state.pageSize, controller.signal)
      const body: unknown = await response.json().catch(() => null)
      if (current !== generation) return
      if (!response.ok) {
        const problem = typeof body === 'object' && body !== null ? body as Record<string, unknown> : {}
        state.error = {
          message: typeof problem.msg === 'string' ? problem.msg : `图片素材加载失败（${response.status}）。`,
      ...
      if (state.selectedFileKey !== null && !state.items.some(item => item.fileKey === state.selectedFileKey)) {
        state.selectedFileKey = null
      }
    } catch (error) {
      if (current === generation && !(error instanceof DOMException && error.name === 'AbortError')) {
        state.error = { message: '暂时无法连接图片素材服务。', requestId: null, status: null }
      }
    } finally {
  ...
  return {
    state, load,
    select(asset) { state.selectedFileKey = asset.fileKey },
    dispose() {
      generation += 1
      controller?.abort()
      controller = null
