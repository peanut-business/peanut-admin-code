...
export const createTaskJobFetchTransport = (options: {
  ...
  const result = async (request: Promise<{ data?: unknown; error?: unknown; response: Response }>): Promise<TaskTransportResult> => {
  ...
  return {
    list(status, page, pageSize, signal) {
      return result(sdk.listTaskJobs({
        credentials: 'include',
    ...
    cancel(jobKey, revision, signal) {
      return result(sdk.cancelTaskJob({
        body: { revision }, credentials: 'include',
        headers: { Accept: 'application/json', 'Content-Type': 'application/json' },
        params: { path: { jobKey } }, signal,
      }))
    },
    retry(jobKey, revision, signal) {
      return result(sdk.retryTaskJob({
        body: { revision }, credentials: 'include',
        headers: { Accept: 'application/json', 'Content-Type': 'application/json' },
        params: { path: { jobKey } }, signal,
      }))
