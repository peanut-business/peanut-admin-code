...
const areas = ['official-file', 'official-import-export', 'official-notification', 'official-task'];
const vueExtension = [{ extension: '.vue', isMixedContent: true, scriptKind: ts.ScriptKind.Deferred }];
function parse(file) {
  const loaded = ts.readConfigFile(file, ts.sys.readFile);
  if (loaded.error) throw new Error(ts.flattenDiagnosticMessageText(loaded.error.messageText, '\n'));
  const config = ts.parseJsonConfigFileContent(loaded.config, ts.sys, dirname(file), {}, file, undefined, vueExtension);
  if (config.errors.length || config.options.strict !== true) {
    throw new Error('OPTIONAL_CONFIG_INVALID: ' + file + '\n' + config.errors.map((error) => ts.flattenDiagnosticMessageText(error.messageText, '\n')).join('\n'));
  }
  return config;
}
...
function requireIncluded(config, expected, label) {
  ...
  for (const file of expected) {
    if (!selected.has(file)) throw new Error('OPTIONAL_SOURCE_EXCLUDED: ' + relative(root, file));
  }
}
...
for (const area of areas) {
  ...
  requireIncluded(parse(resolve(directory, 'tsconfig.json')), productionFiles, area + ' module');
  requireIncluded(tests, testFiles, area + ' tests');
  productionCount += productionFiles.length;
  testCount += testFiles.length;
}
const program = ts.createProgram(tests.fileNames, { ...tests.options, noEmit: true });
...
if (errors.length) {
  ...
  console.log(`OPTIONAL-TYPE-COVERAGE-001 passed: ${productionCount} production files included, ${testCount} test files checked; strict=true`);
}
