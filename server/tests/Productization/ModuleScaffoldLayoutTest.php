...
if (!class_exists(Composer\Autoload\ClassLoader::class, false)) require $root . '/server/vendor/autoload.php';
...
if (!is_string($base) || !str_starts_with($base, $root . '/.local/tmp/')) throw new RuntimeException('LAYOUT_TEST_OWN_CHECKOUT_TMPDIR_REQUIRED');
$temp = $base . '/module-layout-' . bin2hex(random_bytes(6));
mkdir($temp, 0700);
foreach (['server/app/modules', 'web/src/modules', 'server/tests'] as $directory) mkdir($temp . '/' . $directory, 0700, true);
$checks = 0;
...
if ($keep !== null && !str_starts_with($keep, $base . '/')) throw new RuntimeException('LAYOUT_RESULT_OUTSIDE_TASK');
$success = false;
try {
    foreach ([
        ...
        $result = $generator->create($key, $vendor);
        ...
        $loader->setPsr4($namespace, [$temp . '/' . $backend . '/src']); $loader->register(true);
        try {
            ...
            $expect(interface_exists($manifest['contracts']['exports'][0]), 'GENERATED_CONTRACT_COLD_LOAD_FAILED');
    ...
    try { $generator->create('dcs.product', 'dcs'); throw new RuntimeException('DUPLICATE_ACCEPTED'); }
    catch (ModuleScaffoldException $error) { $expect(str_contains($error->getMessage(), 'already exists'), 'DUPLICATE_FAILURE_CHANGED'); }
    $expect(hash_file('sha256', $temp . '/server/app/modules/dcs/product/module.json') === $before, 'DUPLICATE_CHANGED_EXISTING_OUTPUT');
    try { $generator->create('dcs.other', 'Dcs'); throw new RuntimeException('UPPERCASE_VENDOR_ACCEPTED'); }
    catch (ModuleScaffoldException $error) { $expect(str_contains($error->getMessage(), 'vendor is invalid'), 'VENDOR_FAILURE_CHANGED'); }
    ...
    if ($keep !== null) file_put_contents($keep, json_encode($result, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR) . "\n");
    ...
    if ($success && $keep === null) {
        ...
        rmdir($temp);
    } elseif (!$success) fwrite(STDERR, 'Failed fixture retained: ' . $temp . "\n");
}
