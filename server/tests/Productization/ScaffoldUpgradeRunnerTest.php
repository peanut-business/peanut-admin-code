...
{
    $pipes = [];
    $process = proc_open($command, [1 => ['pipe','w'],2 => ['pipe','w']], $pipes, $cwd, $environment + $_ENV);
    ...
    if ($code !== 0) {
        throw new RuntimeException('command failed(' . $code . '): ' . trim((string) $stderr));
    }
...
function scaffoldFileTree(string $root, bool $includeUpgradeState = false): string
{
    ...
    foreach ($iterator as $file) {
        ...
        if (!$includeUpgradeState && ($relative === '.peanut/upgrades' || str_starts_with($relative, '.peanut/upgrades/'))) {
            continue;
        }$rows[] = ($file->isDir() ? 'd' : 'f') . "\0" . $relative . "\0" . ($file->isFile() ? hash_file('sha256', $file->getPathname()) : '-') . "\0" . ($file->getPerms() & 0777);
    }
...
function scaffoldPlanPath(string $project, array $plan): string
{
    return $project . '/' . $plan['plan_path'];
}
function scaffoldFails(callable $callback, string $message): void
{
    try {
        ...
        scaffoldExpect(str_contains($exception->getMessage(), $message), 'unexpected failure: ' . $exception->getMessage());
        return;
    }
...
function scaffoldCopyRelease(string $source, string $target): void
{
    scaffoldCopy(dirname($source), $target);
}

$temporaryRoot = realpath(sys_get_temp_dir());
if ($temporaryRoot === false) {
    throw new RuntimeException('temp root unavailable');
}
$temporary = $temporaryRoot . '/peanut-scaffold-e2e-' . bin2hex(random_bytes(8));
mkdir($temporary, 0700, true);
$fromRelease = $root . '/scaffold/releases/v1.0.0/scaffold-manifest.json';
$toRelease = $root . '/scaffold/releases/v1.1.0/scaffold-manifest.json';
$patchRelease = $root . '/scaffold/releases/v1.1.1/scaffold-manifest.json';
$latestRelease = $root . '/scaffold/releases/v1.1.2/scaffold-manifest.json';
$nextRelease = $root . '/scaffold/releases/v1.1.3/scaffold-manifest.json';
$currentRelease = $root . '/scaffold/releases/v1.1.4/scaffold-manifest.json';
$runtimeRelease = $root . '/scaffold/releases/v1.1.5/scaffold-manifest.json';
$releaseCandidate = $root . '/scaffold/releases/v1.1.6/scaffold-manifest.json';
$productRelease = $root . '/scaffold/releases/v1.1.7/scaffold-manifest.json';
$hotfixRelease = $root . '/scaffold/releases/v1.1.8/scaffold-manifest.json';
$managedSeederRelease = $root . '/scaffold/releases/v1.1.9/scaffold-manifest.json';
try {
    try {
        scaffoldFails(static fn(): null => null, 'SCAFFOLD_TEST_EXPECTED_FAILURE');
        throw new RuntimeException('scaffoldFails accepted a successful callback');
    } catch (RuntimeException $exception) {
        scaffoldExpect($exception->getMessage() === 'expected failure: SCAFFOLD_TEST_EXPECTED_FAILURE', 'scaffoldFails must reject a successful callback');
    }
    ...
    $from = $temporary . '/from-app';
    scaffoldFresh($source, $from);
    ...
    $adoptionFromRelease = $temporary . '/adoption-from-release';
    $adoptionToRelease = $temporary . '/adoption-to-release';
    scaffoldCopyRelease($fromRelease, $adoptionFromRelease);
    scaffoldCopyRelease($toRelease, $adoptionToRelease);
    $adoptionPath = 'server/config/peanut.php';
    $adoptionSource = (string) file_get_contents($source . '/' . $adoptionPath);
    foreach ([$adoptionFromRelease,$adoptionToRelease] as $adoptionRelease) {
        $artifact = $adoptionRelease . '/files/' . $adoptionPath;
        if (!is_dir(dirname($artifact))) {
            mkdir(dirname($artifact), 0775, true);
        }file_put_contents($artifact, $adoptionSource);
        $manifestPath = $adoptionRelease . '/scaffold-manifest.json';
    ...
    $adoptionPlan = $runner->preflight($from, $adoptionFromRelease . '/scaffold-manifest.json', $adoptionToRelease . '/scaffold-manifest.json');
    ...
    $adoptionBefore = scaffoldFileTree($from);
    scaffoldFails(fn() => $runner->apply($from, scaffoldPlanPath($from, $adoptionPlan)), 'SCAFFOLD_PLAN_BLOCKED');
    scaffoldExpect(hash_equals($adoptionBefore, scaffoldFileTree($from)), 'blocked ownership transition must not write the product tree');
    ...
    $deletionBefore = scaffoldFileTree($from);
    scaffoldFails(fn() => $runner->apply($from, scaffoldPlanPath($from, $deletionPlan)), 'SCAFFOLD_PLAN_BLOCKED');
    scaffoldExpect(hash_equals($deletionBefore, scaffoldFileTree($from)), 'blocked app-owned deletion must not write the product tree');
    scaffoldDelete($from . '/.peanut/upgrades');

    $missingManifestPath = $from . '/.peanut/application-manifest.json';
    $missingManifestBytes = (string) file_get_contents($missingManifestPath);
    $missingManifest = json_decode($missingManifestBytes, true, 512, JSON_THROW_ON_ERROR);
    $missingManifest['files'] = array_values(array_filter($missingManifest['files'], static fn(array $file): bool => ($file['path'] ?? null) !== 'README.md'));
    file_put_contents($missingManifestPath, json_encode($missingManifest, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR) . "\n");
    $missingPlan = $runner->preflight($from, $fromRelease, $toRelease);
    scaffoldExpect($missingPlan['status'] === 'blocked' && count(array_filter($missingPlan['actions'], static fn(array $action): bool => $action['reason'] === 'managed_adoption_required')) === 1, 'a missing managed adoption record must block');
    $missingBefore = scaffoldFileTree($from);
    scaffoldFails(fn() => $runner->apply($from, scaffoldPlanPath($from, $missingPlan)), 'SCAFFOLD_PLAN_BLOCKED');
    scaffoldExpect(hash_equals($missingBefore, scaffoldFileTree($from)), 'blocked missing adoption record must not write the product tree');
    file_put_contents($missingManifestPath, $missingManifestBytes);
    scaffoldDelete($from . '/.peanut/upgrades');

    $newTargetRelease = $temporary . '/new-target-release';
    scaffoldCopyRelease($toRelease, $newTargetRelease);
    $newPath = 'host/new-managed.txt';
    $newArtifact = $newTargetRelease . '/files/' . $newPath;
    if (!is_dir(dirname($newArtifact))) {
        mkdir(dirname($newArtifact), 0775, true);
    }file_put_contents($newArtifact, "new host file\n");
    $newManifestPath = $newTargetRelease . '/scaffold-manifest.json';
    $newManifest = json_decode((string) file_get_contents($newManifestPath), true, 512, JSON_THROW_ON_ERROR);
    $newManifest['files'][] = ['path' => $newPath,'source' => 'files/' . $newPath,'template_sha256' => hash('sha256', "new host file\n"),'classification' => 'managed','transform' => 'tokens','mode' => 0644,'policy' => 'managed','owner' => 'host'];
    file_put_contents($newManifestPath, json_encode($newManifest, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR) . "\n");
    $newPlan = $runner->preview($from, $fromRelease, $newManifestPath);
    scaffoldExpect($newPlan['status'] === 'ready' && count(array_filter($newPlan['actions'], static fn(array $action): bool => ($action['path'] ?? null) === 'host/new-managed.txt' && ($action['action'] ?? null) === 'create')) === 1, 'a target-only path without an instance file must remain creatable');

    $appOwnedPath = 'server/config/peanut.php';
    file_put_contents($from . '/' . $appOwnedPath, (string) file_get_contents($from . '/' . $appOwnedPath) . "\n// application customization\n");
    ...
    $apply = $runner->apply($from, scaffoldPlanPath($from, $plan));
    $verify = $runner->verify($from, scaffoldPlanPath($from, $plan));
    ...
    $toSource = $temporary . '/to-source';
    scaffoldRun(['git','clone','--quiet','--no-local','--no-checkout',$root,$toSource]);
    scaffoldRun(['git','checkout','--quiet','--detach',$toIdentity['source_commit']], $toSource);
    $toApplication = $temporary . '/to-app';
    scaffoldFresh($toSource, $toApplication);
    $toApplicationManifest = json_decode((string) file_get_contents($toApplication . '/.peanut/application-manifest.json'), true, 512, JSON_THROW_ON_ERROR);
    ...
    foreach ($toApplicationManifest['files'] as $file) {
        if (!in_array($file['classification'], ['managed','generated-managed'], true)) {
            continue;
        }$upgraded = $from . '/' . $file['path'];
        $generated = $toApplication . '/' . $file['path'];
        scaffoldExpect(is_file($upgraded) && hash_equals((string) hash_file('sha256', $generated), (string) hash_file('sha256', $upgraded)) && ((fileperms($generated) & 0777) === (fileperms($upgraded) & 0777)), 'upgraded managed tree must exactly equal target create-app: ' . $file['path']);
    }
    $idempotentBefore = scaffoldFileTree($from, true);
    $again = $runner->apply($from, scaffoldPlanPath($from, $plan));
    $verifyAgain = $runner->verify($from, scaffoldPlanPath($from, $plan));
    ...
    $appliedManifestPath = $from . '/.peanut/application-manifest.json';
    $appliedManifestBytes = (string) file_get_contents($appliedManifestPath);
    $appliedManifest = json_decode($appliedManifestBytes, true, 512, JSON_THROW_ON_ERROR);
    $managedDriftFile = null;
    foreach ($appliedManifest['files'] as $file) {
        if (in_array($file['classification'], ['managed','generated-managed'], true) && ($file['mode'] ?? null) === 0644) {
            $managedDriftFile = $file;
            break;
        }
    }
    scaffoldExpect(is_array($managedDriftFile), 'an ordinary managed file is required for repeat drift checks');
    $managedDriftPath = $from . '/' . $managedDriftFile['path'];
    $managedDriftBytes = (string) file_get_contents($managedDriftPath);
    ...
    file_put_contents($managedDriftPath, $managedDriftBytes);
    chmod($managedDriftPath, 0600);
    $managedModeTree = scaffoldFileTree($from, true);
    ...
    $appOwnedBytes = (string) file_get_contents($from . '/' . $appOwnedPath);
    ...
    file_put_contents($from . '/' . $appOwnedPath, $appOwnedBytes);
    $identityDrift = $appliedManifest;
    $identityDrift['template']['source_commit'] = str_repeat('0', 40);
    file_put_contents($appliedManifestPath, json_encode($identityDrift, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR) . "\n");
    ...
    scaffoldCopy($temporary . '/from-app', $blocked);
    // Restore a real v1 project for independent scenarios.
    scaffoldDelete($blocked);
    scaffoldCopy($temporary . '/from-app', $blocked);
    ...
    scaffoldDelete($blocked);
    scaffoldFresh($source, $blocked);
    $changed = 'scripts/scaffold-upgrade';
    file_put_contents($blocked . '/' . $changed, (string) file_get_contents($blocked . '/' . $changed) . "\n# local managed edit\n");
    $beforeBlocked = scaffoldFileTree($blocked);
    $blockedPlan = $runner->preflight($blocked, $fromRelease, $toRelease);
    scaffoldExpect($blockedPlan['status'] === 'blocked', 'both-sides managed change must block');
    ...
    scaffoldFresh($source, $stale);
    $stalePlan = $runner->preflight($stale, $fromRelease, $toRelease);
    file_put_contents($stale . '/README.md', (string) file_get_contents($stale . '/README.md') . "\nchanged after plan\n");
    ...
    scaffoldFresh($source, $tampered);
    $tamperedPlan = $runner->preflight($tampered, $fromRelease, $toRelease);
    $tamperedPath = scaffoldPlanPath($tampered, $tamperedPlan);
    $tamperedData = json_decode((string) file_get_contents($tamperedPath), true, 512, JSON_THROW_ON_ERROR);
    $tamperedData['actions'][0]['mode'] = 0600;
    file_put_contents($tamperedPath, json_encode($tamperedData, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR));
    scaffoldFails(fn() => $runner->apply($tampered, $tamperedPath), 'SCAFFOLD_PLAN_CHECKSUM_DRIFT');

    $fault = $temporary . '/fault';
    scaffoldFresh($source, $fault);
    $faultBefore = scaffoldFileTree($fault);
    $faultPlan = $runner->preflight($fault, $fromRelease, $toRelease);
    $faultRunner = new ScaffoldUpgradeRunner(1);
    scaffoldFails(fn() => $faultRunner->apply($fault, scaffoldPlanPath($fault, $faultPlan)), 'SCAFFOLD_FAULT_INJECTED');
    scaffoldExpect(!hash_equals($faultBefore, scaffoldFileTree($fault)), 'fault injection must happen after a real replacement');
    $recover = $runner->recover($fault, scaffoldPlanPath($fault, $faultPlan));
    scaffoldExpect(hash_equals($faultBefore, scaffoldFileTree($fault)), 'recovery must restore the exact pre-apply tree and modes');
    $recoverAgain = $runner->recover($fault, scaffoldPlanPath($fault, $faultPlan));
    scaffoldExpect($recoverAgain['idempotent'] === true && $recover['status'] === 'recovered', 'recovery must be idempotent');

    $securityRelease = $temporary . '/security-release';
    scaffoldCopyRelease($toRelease, $securityRelease);
    $securityManifest = $securityRelease . '/scaffold-manifest.json';
    $securityData = json_decode((string) file_get_contents($securityManifest), true, 512, JSON_THROW_ON_ERROR);
    $securityData['files'][0]['path'] = '../escape';
    file_put_contents($securityManifest, json_encode($securityData, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR));
    scaffoldFails(fn() => $runner->preflight($fault, $fromRelease, $securityManifest), 'SCAFFOLD_PATH_OUTSIDE_PROJECT');
    scaffoldDelete($securityRelease);
    scaffoldCopyRelease($toRelease, $securityRelease);
    $securityManifest = $securityRelease . '/scaffold-manifest.json';
    $securityData = json_decode((string) file_get_contents($securityManifest), true, 512, JSON_THROW_ON_ERROR);
    $securityData['files'][0]['policy'] = 'unknown';
    file_put_contents($securityManifest, json_encode($securityData, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR));
    ...
    $securityManifest = $securityRelease . '/scaffold-manifest.json';
    $securityData = json_decode((string) file_get_contents($securityManifest), true, 512, JSON_THROW_ON_ERROR);
    $securityData['files'][0]['transform'] = 'unknown';
    file_put_contents($securityManifest, json_encode($securityData, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR));
    ...
    scaffoldFresh($source, $symlink);
    $outside = $temporary . '/outside';
    file_put_contents($outside, 'outside');
    unlink($symlink . '/scripts/scaffold-upgrade');
    symlink($outside, $symlink . '/scripts/scaffold-upgrade');
    ...
    scaffoldFresh($source, $hardlink);
    link($hardlink . '/scripts/scaffold-upgrade', $hardlink . '/hardlink-alias');
    scaffoldFails(fn() => $runner->preflight($hardlink, $fromRelease, $toRelease), 'SCAFFOLD_PATH_HARDLINK_REJECTED');

    $drift = $temporary . '/drift';
    scaffoldFresh($source, $drift);
    $driftRelease = $temporary . '/drift-release';
    scaffoldCopyRelease($toRelease, $driftRelease);
    $driftManifest = $driftRelease . '/scaffold-manifest.json';
    $driftPlan = $runner->preflight($drift, $fromRelease, $driftManifest);
    ...
    $patchSource = $temporary . '/patch-from-source';
    scaffoldRun(['git','clone','--quiet','--no-local','--no-checkout',$root,$patchSource]);
    scaffoldRun(['git','checkout','--quiet','--detach',$toIdentity['source_commit']], $patchSource);
    $patchApp = $temporary . '/patch-app';
    scaffoldFresh($patchSource, $patchApp);
    $patchBefore = scaffoldFileTree($patchApp);
    ...
    $patchActions = [];
    foreach ($patchPlan['actions'] as $action) {
        $patchActions[$action['path']] = $action['action'];
    }
    ...
    $patchApply = $runner->apply($patchApp, scaffoldPlanPath($patchApp, $patchPlan));
    $patchVerify = $runner->verify($patchApp, scaffoldPlanPath($patchApp, $patchPlan));
    ...
    $latestFromSource = $temporary . '/latest-from-source';
    scaffoldRun(['git','clone','--quiet','--no-local','--no-checkout',$root,$latestFromSource]);
    scaffoldRun(['git','checkout','--quiet','--detach',$patchIdentity['source_commit']], $latestFromSource);
    $latestApp = $temporary . '/latest-app';
    scaffoldFresh($latestFromSource, $latestApp);
    $latestAppOwnedPath = 'server/config/peanut.php';
    file_put_contents($latestApp . '/' . $latestAppOwnedPath, (string) file_get_contents($latestApp . '/' . $latestAppOwnedPath) . "\n// v1.1.2 preservation proof\n");
    $latestAppOwnedDigest = hash_file('sha256', $latestApp . '/' . $latestAppOwnedPath);
    $latestApplicationManifestPath = $latestApp . '/.peanut/application-manifest.json';
    $latestApplicationManifest = json_decode((string) file_get_contents($latestApplicationManifestPath), true, 512, JSON_THROW_ON_ERROR);
    $generationSource = ['commit' => str_repeat('c', 40),'tree' => str_repeat('d', 40),'inventory_sha256' => str_repeat('e', 64)];
    $latestApplicationManifest['generation_source'] = $generationSource;
    ...
    $latestBefore = scaffoldFileTree($latestApp);
    $latestPlan = $runner->preflight($latestApp, $patchRelease, $latestRelease);
    ...
    $latestApply = $runner->apply($latestApp, scaffoldPlanPath($latestApp, $latestPlan));
    $latestVerify = $runner->verify($latestApp, scaffoldPlanPath($latestApp, $latestPlan));
    ...
    $nextFromSource = $temporary . '/next-from-source';
    scaffoldRun(['git','clone','--quiet','--no-local','--no-checkout',$root,$nextFromSource]);
    scaffoldRun(['git','checkout','--quiet','--detach',SCAFFOLD_V1_1_2_CREATE_COMMIT], $nextFromSource);
    $nextApp = $temporary . '/next-app';
    scaffoldFresh($nextFromSource, $nextApp);
    $nextAppOwnedPath = 'server/config/peanut.php';
    file_put_contents($nextApp . '/' . $nextAppOwnedPath, (string) file_get_contents($nextApp . '/' . $nextAppOwnedPath) . "\n// v1.1.3 preservation proof\n");
    $nextAppOwnedDigest = hash_file('sha256', $nextApp . '/' . $nextAppOwnedPath);
    $nextApplicationManifestPath = $nextApp . '/.peanut/application-manifest.json';
    $nextApplicationManifest = json_decode((string) file_get_contents($nextApplicationManifestPath), true, 512, JSON_THROW_ON_ERROR);
    $nextGenerationSource = ['commit' => str_repeat('f', 40),'tree' => str_repeat('1', 40),'inventory_sha256' => str_repeat('2', 64)];
    $nextApplicationManifest['generation_source'] = $nextGenerationSource;
    ...
    $nextBefore = scaffoldFileTree($nextApp);
    $nextPlan = $runner->preflight($nextApp, $latestRelease, $nextRelease);
    ...
    $nextApply = $runner->apply($nextApp, scaffoldPlanPath($nextApp, $nextPlan));
    $nextVerify = $runner->verify($nextApp, scaffoldPlanPath($nextApp, $nextPlan));
    ...
    $legacySource = $temporary . '/legacy-version-source';
    scaffoldRun(['git','clone','--quiet','--no-local','--no-checkout',$root,$legacySource]);
    scaffoldRun(['git','checkout','--quiet','--detach',$legacyIdentity['source_commit']], $legacySource);
    $legacyApp = $temporary . '/legacy-version-app';
    scaffoldFreshAdopted($legacySource, $nextRelease, $legacyApp);
    $legacyManifestPath = $legacyApp . '/.peanut/application-manifest.json';
    $legacyManifest = json_decode((string) file_get_contents($legacyManifestPath), true, 512, JSON_THROW_ON_ERROR);
    ...
    $legacyMetadataPath = $legacyApp . '/RELEASE_METADATA.json';
    $legacyMetadata = json_decode((string) file_get_contents($legacyMetadataPath), true, 512, JSON_THROW_ON_ERROR);
    $legacyMetadata['version'] = '2.4.6';
    file_put_contents($legacyMetadataPath, json_encode($legacyMetadata, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR) . "\n");
    scaffoldInstallVersionContract($legacyApp, '2.4.6');
    $legacyAppOwnedDigest = scaffoldOwnedTree($legacyApp, $legacyManifest, 'app-owned');
    $legacyUniappDigest = hash_file('sha256', $legacyApp . '/uniapp/src/manifest.json');
    ...
    $legacyApply = $runner->apply($legacyApp, scaffoldPlanPath($legacyApp, $legacyPlan));
    $legacyVerify = $runner->verify($legacyApp, scaffoldPlanPath($legacyApp, $legacyPlan));
    ...
    foreach (['web/package.json','pc/package.json','uniapp/package.json','server/config/project.php'] as $versionPath) {
        scaffoldExpect(str_contains((string) file_get_contents($legacyApp . '/' . $versionPath), '2.4.6'), 'managed application version surface was not preserved: ' . $versionPath);
    }

    $ambiguousApp = $temporary . '/legacy-version-ambiguous';
    scaffoldFreshAdopted($legacySource, $nextRelease, $ambiguousApp);
    $ambiguousMetadataPath = $ambiguousApp . '/RELEASE_METADATA.json';
    $ambiguousMetadata = json_decode((string) file_get_contents($ambiguousMetadataPath), true, 512, JSON_THROW_ON_ERROR);
    $ambiguousMetadata['application'] = ['version' => '9.9.9'];
    file_put_contents($ambiguousMetadataPath, json_encode($ambiguousMetadata, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR) . "\n");
    ...
    $runtimeSource = $temporary . '/runtime-source';
    scaffoldRun(['git','clone','--quiet','--no-local','--no-checkout',$root,$runtimeSource]);
    scaffoldRun(['git','checkout','--quiet','--detach',$currentIdentity['source_commit']], $runtimeSource);
    $runtimeApp = $temporary . '/runtime-app';
    scaffoldFreshAdopted($runtimeSource, $currentRelease, $runtimeApp);
    ...
    $runtimeAppOwnedPath = 'server/config/peanut.php';
    file_put_contents($runtimeApp . '/' . $runtimeAppOwnedPath, (string) file_get_contents($runtimeApp . '/' . $runtimeAppOwnedPath) . "\n// v1.1.5 preservation proof\n");
    $runtimeAppOwnedDigest = hash_file('sha256', $runtimeApp . '/' . $runtimeAppOwnedPath);
    $runtimeBefore = scaffoldFileTree($runtimeApp);
    $runtimePlan = $runner->preflight($runtimeApp, $currentRelease, $runtimeRelease);
    scaffoldExpect($runtimePlan['status'] === 'ready' && $runtimePlan['summary']['conflicts'] === 0, 'v1.1.4 to v1.1.5 plan must be ready');
    $runtimeAutomatic = [];
    foreach ($runtimePlan['actions'] as $action) {
        if (in_array($action['action'], ['create','delete','replace','regenerate'], true)) {
            $runtimeAutomatic[$action['path']] = $action['action'];
        }
    }ksort($runtimeAutomatic, SORT_STRING);
    ...
    $runtimeApply = $runner->apply($runtimeApp, scaffoldPlanPath($runtimeApp, $runtimePlan));
    $runtimeVerify = $runner->verify($runtimeApp, scaffoldPlanPath($runtimeApp, $runtimePlan));
    ...
    $runtimeRecover = $runner->recover($runtimeApp, scaffoldPlanPath($runtimeApp, $runtimePlan));
    scaffoldExpect($runtimeRecover['status'] === 'recovered' && hash_equals($runtimeBefore, scaffoldFileTree($runtimeApp)), 'v1.1.5 recovery must restore the exact v1.1.4 tree');

    $runtimeIdentity = json_decode((string) file_get_contents($runtimeRelease), true, 512, JSON_THROW_ON_ERROR)['release'];
    $releaseCandidateSource = $temporary . '/release-candidate-source';
    scaffoldRun(['git','clone','--quiet','--no-local','--no-checkout',$root,$releaseCandidateSource]);
    scaffoldRun(['git','checkout','--quiet','--detach',$runtimeIdentity['source_commit']], $releaseCandidateSource);
    $releaseCandidateApp = $temporary . '/release-candidate-app';
    scaffoldFreshAdopted($releaseCandidateSource, $runtimeRelease, $releaseCandidateApp);
    $releaseCandidateAppOwnedPath = 'server/config/peanut.php';
    file_put_contents($releaseCandidateApp . '/' . $releaseCandidateAppOwnedPath, (string) file_get_contents($releaseCandidateApp . '/' . $releaseCandidateAppOwnedPath) . "\n// v1.1.6 preservation proof\n");
    $releaseCandidateAppOwnedDigest = hash_file('sha256', $releaseCandidateApp . '/' . $releaseCandidateAppOwnedPath);
    $releaseCandidateBefore = scaffoldFileTree($releaseCandidateApp);
    $releaseCandidatePlan = $runner->preflight($releaseCandidateApp, $runtimeRelease, $releaseCandidate);
    ...
    $releaseCandidateAutomatic = [];
    foreach ($releaseCandidatePlan['actions'] as $action) {
        if (in_array($action['action'], ['create','delete','replace','regenerate'], true)) {
            $releaseCandidateAutomatic[$action['path']] = $action['action'];
        }
    }ksort($releaseCandidateAutomatic, SORT_STRING);
    ...
    $releaseCandidateApply = $runner->apply($releaseCandidateApp, scaffoldPlanPath($releaseCandidateApp, $releaseCandidatePlan));
    $releaseCandidateVerify = $runner->verify($releaseCandidateApp, scaffoldPlanPath($releaseCandidateApp, $releaseCandidatePlan));
    ...
    $releaseCandidateRecover = $runner->recover($releaseCandidateApp, scaffoldPlanPath($releaseCandidateApp, $releaseCandidatePlan));
    scaffoldExpect($releaseCandidateRecover['status'] === 'recovered' && hash_equals($releaseCandidateBefore, scaffoldFileTree($releaseCandidateApp)), 'v1.1.6 recovery must restore the exact v1.1.5 tree');

    $releaseCandidateIdentity = json_decode((string) file_get_contents($releaseCandidate), true, 512, JSON_THROW_ON_ERROR)['release'];
    $productReleaseSource = $temporary . '/product-release-source';
    scaffoldRun(['git','clone','--quiet','--no-local','--no-checkout',$root,$productReleaseSource]);
    scaffoldRun(['git','checkout','--quiet','--detach',$releaseCandidateIdentity['source_commit']], $productReleaseSource);
    $productReleaseApp = $temporary . '/product-release-app';
    scaffoldFreshAdopted($productReleaseSource, $releaseCandidate, $productReleaseApp);
    $productReleaseAppOwnedPath = 'server/config/peanut.php';
    file_put_contents($productReleaseApp . '/' . $productReleaseAppOwnedPath, (string) file_get_contents($productReleaseApp . '/' . $productReleaseAppOwnedPath) . "\n// v1.1.7 preservation proof\n");
    $productReleaseAppOwnedDigest = hash_file('sha256', $productReleaseApp . '/' . $productReleaseAppOwnedPath);
    $productReleasePlan = $runner->preflight($productReleaseApp, $releaseCandidate, $productRelease);
    scaffoldExpect($productReleasePlan['status'] === 'ready' && $productReleasePlan['summary']['conflicts'] === 0, 'v1.1.6 to v1.1.7 plan must be ready');
    $productReleaseApply = $runner->apply($productReleaseApp, scaffoldPlanPath($productReleaseApp, $productReleasePlan));
    $productReleaseVerify = $runner->verify($productReleaseApp, scaffoldPlanPath($productReleaseApp, $productReleasePlan));
    ...
    $hotfixSource = $temporary . '/hotfix-source';
    scaffoldRun(['git','clone','--quiet','--no-local','--no-checkout',$root,$hotfixSource]);
    scaffoldRun(['git','checkout','--quiet','--detach',$productReleaseIdentity['source_commit']], $hotfixSource);
    $hotfixApp = $temporary . '/hotfix-app';
    scaffoldFreshAdopted($hotfixSource, $productRelease, $hotfixApp);
    $hotfixSeederDigest = hash_file('sha256', $hotfixApp . '/scripts/seed-demo-data');
    $hotfixBefore = scaffoldFileTree($hotfixApp);
    $hotfixPlan = $runner->preflight($hotfixApp, $productRelease, $hotfixRelease);
    scaffoldExpect($hotfixPlan['status'] === 'ready' && $hotfixPlan['summary']['conflicts'] === 0, 'v1.1.7 to v1.1.8 plan must be ready');
    $hotfixApply = $runner->apply($hotfixApp, scaffoldPlanPath($hotfixApp, $hotfixPlan));
    $hotfixVerify = $runner->verify($hotfixApp, scaffoldPlanPath($hotfixApp, $hotfixPlan));
    ...
    $hotfixRecover = $runner->recover($hotfixApp, scaffoldPlanPath($hotfixApp, $hotfixPlan));
    scaffoldExpect($hotfixRecover['status'] === 'recovered' && hash_equals($hotfixBefore, scaffoldFileTree($hotfixApp)), 'v1.1.8 recovery must restore the exact v1.1.7 tree');

    $hotfixIdentity = json_decode((string) file_get_contents($hotfixRelease), true, 512, JSON_THROW_ON_ERROR)['release'];
    $managedSeederSource = $temporary . '/managed-seeder-source';
    scaffoldRun(['git','clone','--quiet','--no-local','--no-checkout',$root,$managedSeederSource]);
    scaffoldRun(['git','checkout','--quiet','--detach',$hotfixIdentity['source_commit']], $managedSeederSource);
    $managedSeederApp = $temporary . '/managed-seeder-app';
    scaffoldFreshAdopted($managedSeederSource, $hotfixRelease, $managedSeederApp);
    ...
    $managedSeederWrapperDigest = hash_file('sha256', $managedSeederApp . '/scripts/seed-demo-data');
    $managedSeederBefore = scaffoldFileTree($managedSeederApp);
    $managedSeederPlan = $runner->preflight($managedSeederApp, $hotfixRelease, $managedSeederRelease);
    scaffoldExpect($managedSeederPlan['status'] === 'ready' && $managedSeederPlan['summary']['conflicts'] === 0, 'v1.1.8 to v1.1.9 plan must be ready');
    $managedSeederApply = $runner->apply($managedSeederApp, scaffoldPlanPath($managedSeederApp, $managedSeederPlan));
    $managedSeederVerify = $runner->verify($managedSeederApp, scaffoldPlanPath($managedSeederApp, $managedSeederPlan));
    ...
    $managedSeederRecover = $runner->recover($managedSeederApp, scaffoldPlanPath($managedSeederApp, $managedSeederPlan));
    scaffoldExpect($managedSeederRecover['status'] === 'recovered' && hash_equals($managedSeederBefore, scaffoldFileTree($managedSeederApp)), 'v1.1.9 recovery must restore the exact v1.1.8 tree');

    $releaseCheckTemporary = $temporary . '/release-check-tmp';
    mkdir($releaseCheckTemporary, 0700, true);
    $_ENV['TMPDIR'] = $releaseCheckTemporary;
    ...
    $toManifest = json_decode((string) file_get_contents($toRelease), true, 512, JSON_THROW_ON_ERROR);
    $toCheck = scaffoldRun(['php',$root . '/scripts/build-scaffold-release','--version=1.1.0','--source-commit=' . $toManifest['release']['source_commit'],'--output=' . $root . '/scaffold/releases/v1.1.0','--check']);
    $patchManifest = json_decode((string) file_get_contents($patchRelease), true, 512, JSON_THROW_ON_ERROR);
    $patchCheck = scaffoldRun(['php',$root . '/scripts/build-scaffold-release','--version=1.1.1','--source-commit=' . $patchManifest['release']['source_commit'],'--output=' . $root . '/scaffold/releases/v1.1.1','--check']);
    $latestManifest = json_decode((string) file_get_contents($latestRelease), true, 512, JSON_THROW_ON_ERROR);
    $latestCheck = scaffoldRun(['php',$root . '/scripts/build-scaffold-release','--version=1.1.2','--source-commit=' . $latestManifest['release']['source_commit'],'--output=' . $root . '/scaffold/releases/v1.1.2','--check']);
    $nextManifest = json_decode((string) file_get_contents($nextRelease), true, 512, JSON_THROW_ON_ERROR);
    $nextCheck = scaffoldRun(['php',$root . '/scripts/build-scaffold-release','--version=1.1.3','--source-commit=' . $nextManifest['release']['source_commit'],'--output=' . $root . '/scaffold/releases/v1.1.3','--check']);
    $currentManifest = json_decode((string) file_get_contents($currentRelease), true, 512, JSON_THROW_ON_ERROR);
    $currentCheck = scaffoldRun(['php',$root . '/scripts/build-scaffold-release','--version=1.1.4','--source-commit=' . $currentManifest['release']['source_commit'],'--output=' . $root . '/scaffold/releases/v1.1.4','--check']);
    $runtimeManifest = json_decode((string) file_get_contents($runtimeRelease), true, 512, JSON_THROW_ON_ERROR);
    $runtimeCheck = scaffoldRun(['php',$root . '/scripts/build-scaffold-release','--version=1.1.5','--source-commit=' . $runtimeManifest['release']['source_commit'],'--output=' . $root . '/scaffold/releases/v1.1.5','--check']);
    $releaseCandidateManifest = json_decode((string) file_get_contents($releaseCandidate), true, 512, JSON_THROW_ON_ERROR);
    $releaseCandidateCheck = scaffoldRun(['php',$root . '/scripts/build-scaffold-release','--version=1.1.6','--source-commit=' . $releaseCandidateManifest['release']['source_commit'],'--output=' . $root . '/scaffold/releases/v1.1.6','--check']);
    $productReleaseManifest = json_decode((string) file_get_contents($productRelease), true, 512, JSON_THROW_ON_ERROR);
    $productReleaseCheck = scaffoldRun(['php',$root . '/scripts/build-scaffold-release','--version=1.1.7','--source-commit=' . $productReleaseManifest['release']['source_commit'],'--output=' . $root . '/scaffold/releases/v1.1.7','--check']);
    $hotfixManifest = json_decode((string) file_get_contents($hotfixRelease), true, 512, JSON_THROW_ON_ERROR);
    $hotfixCheck = scaffoldRun(['php',$root . '/scripts/build-scaffold-release','--version=1.1.8','--source-commit=' . $hotfixManifest['release']['source_commit'],'--output=' . $root . '/scaffold/releases/v1.1.8','--check']);
    $managedSeederManifest = json_decode((string) file_get_contents($managedSeederRelease), true, 512, JSON_THROW_ON_ERROR);
    $managedSeederCheck = scaffoldRun(['php',$root . '/scripts/build-scaffold-release','--version=1.1.9','--source-commit=' . $managedSeederManifest['release']['source_commit'],'--output=' . $root . '/scaffold/releases/v1.1.9','--check']);
