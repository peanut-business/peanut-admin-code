...
function moduleCreateRun(string $serverRoot, array $arguments): array
{
    ...
    if (!is_resource($process)) {
        throw new RuntimeException('module:create process could not start');
    }
...
function moduleCreateRunViteDiscovery(string $projectRoot): array
{
    ...
    if (!is_resource($process)) {
        throw new RuntimeException('Vite discovery process could not start');
    }
    $stdout = stream_get_contents($pipes[1]);
    $stderr = stream_get_contents($pipes[2]);
    fclose($pipes[1]);
...
try {
    $official = moduleCreateRun($serverRoot, [$officialKey, '--client=admin-web']);
    ...
    $custom = moduleCreateRun($serverRoot, [$customKey, '--vendor=acme', '--client=admin-web']);
    ...
    $expectedFrontendFiles = ['contribution.ts', 'package.json'];
    $expectedTestFiles = ['ModuleStructureTest.php', 'SECURITY.md'];
    foreach ([
        [$officialBackend, 'Generated' . ucfirst($suffix)],
        [$customBackend, 'CustomGenerated' . ucfirst($suffix)],
    ] as [$root, $moduleName]) {
        $expectedBackendFiles = [
            'module.json', 'src/ModuleProvider.php',
            'route/app.php',
    ...
    foreach ([$officialTests, $customTests] as $root) {
        ...
        $securityTest = (string) file_get_contents($root . '/SECURITY.md');
    ...
    moduleCreateExpect(($officialManifest['contracts']['exports'] ?? null) === [], 'generator must not publish an invented empty command interface');
    ...
    moduleCreateExpect(!str_contains((string) file_get_contents($customTests . '/ModuleStructureTest.php'), '${'), 'test template placeholder remains');
