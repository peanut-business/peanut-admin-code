...
final readonly class KernelMigrationRunner
{
    ...
    private function manager(): Manager
    {
        $config = new Config([
            'paths' => [
                'migrations' => [
                    dirname((new \ReflectionClass(\PeanutAdmin\Kernel\Migration\ModuleSchema::class))->getFileName(), 3) . '/database/migrations',
                    dirname(__DIR__, 6) . '/database/kernel-migrations',
                ],
            ],
            'environments' => [
