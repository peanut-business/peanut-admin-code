...
$failures = [];
$slice = $argv[1] ?? 'all';
if (!in_array($slice, ['all', 'd01', 'd02', 'd03'], true)) throw new InvalidArgumentException('Expected all, d01, d02 or d03 slice');
...
): array
{
    ...
    $requests++;
    $environment = getenv();
    foreach (peanutBackendEnvironmentKeys() as $key) unset($environment[$key], $environment['PHP_' . $key]);
    ...
    if ($code !== 0 || !is_array($result)) throw new RuntimeException('HTTP worker failed for ' . $path . ': ' . substr($stderr, 0, 1200));
    return $result;
}
...
function seedHttpCode(PDO $pdo, string $scene, string $mobile, string $code): int
{
    ...
    if ($sceneId < 1) throw new RuntimeException('HTTP verification scene missing: ' . $scene);
...
try {
    ...
    if ($exists->fetchColumn() !== false) throw new RuntimeException('D03 HTTP database must not pre-exist');
    ...
    foreach (KernelSchema::tableNames() as $table) $pdo->exec(KernelSchema::createSql($table));
    ...
    foreach ([['nickname',['bad']],['avatar',['bad']],['sex','3'],['birthday','2025-02-29'],['email','bad-email']] as [$field,$value]) httpError(stabilizationHttp('POST','api/user/setInfo',compact('field','value'),$token), 'MEMBER_PROFILE_VALUE_INVALID', 'D02 invalid ' . $field);
    ...
    for ($attempt=0; $attempt<5; $attempt++) stabilizationHttp('POST','api/login/mobile',['mobile'=>'13800000701','code'=>'wrong']);
    ...
    if (in_array($slice, ['all', 'd02'], true)) httpExpect(stabilizationHttp('POST','api/user/setInfo',['field'=>'nickname','value'=>'Disabled'],$token)['status'] === 403, 'D02 disabled member module was accepted');
    if (in_array($slice, ['all', 'd01'], true)) httpExpect(stabilizationHttp('POST','api/login/mobile',['mobile'=>'13800000701','code'=>'918273'])['status'] === 403, 'D01 disabled member module was accepted');
} finally {
    if ($created) $admin->exec("DROP DATABASE `{$database}`");
