...
final class TenantAuthService
{
    ...
    public function selectTenant(
        ...
        $this->assertPrefix($challengeToken, TokenIssuer::CHALLENGE_PREFIX, 'AUTH_CHALLENGE_INVALID');
        $now = $this->clock->now();
        $result = Db::transaction(function () use (
            $challengeToken,
            $tenantId,
    ...
    public function refresh(
        ...
        $this->assertTenantTokenPrefix($refreshToken, TokenIssuer::TENANT_REFRESH_PREFIX);
        $now = $this->clock->now();
        $result = Db::transaction(function () use (
            $refreshToken,
            $ipAddress,
    ...
    {
        $this->assertTenantTokenPrefix($accessToken, TokenIssuer::TENANT_ACCESS_PREFIX);
        $now = $this->clock->now();
    ...
    {
        if (str_starts_with($token, TokenIssuer::PLATFORM_ACCESS_PREFIX) || str_starts_with($token, TokenIssuer::PLATFORM_REFRESH_PREFIX)) {
