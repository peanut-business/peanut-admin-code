...
final class PlatformAuthService
{
    ...
    public function refresh(
        ...
        $this->assertPlatformPrefix($refreshToken, TokenIssuer::PLATFORM_REFRESH_PREFIX);
        $now = $this->clock->now();
        $result = Db::transaction(function () use (
            $refreshToken,
            $ipAddress,
    ...
    {
        $this->assertPlatformPrefix($accessToken, TokenIssuer::PLATFORM_ACCESS_PREFIX);
        $now = $this->clock->now();
    ...
    {
        if (str_starts_with($token, TokenIssuer::TENANT_ACCESS_PREFIX) || str_starts_with($token, TokenIssuer::TENANT_REFRESH_PREFIX)) {
