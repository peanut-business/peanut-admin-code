...
namespace app\modules\official\import_export\engine\Persistence;

use DateTimeImmutable;
use DateTimeZone;
use JsonException;
use app\modules\official\import_export\engine\Application\ImportExportException;
use app\modules\official\import_export\engine\Application\OperationRecord;
use app\modules\official\import_export\engine\Contract\RowIssue;
use app\modules\official\import_export\engine\Persistence\Model\ImportExportOperationRecord;
use app\modules\official\import_export\engine\Persistence\Model\ImportExportRowErrorRecord;
...
final class ImportExportStore
{
    ...
    {
        $this->assertStorageMode();
        $row = $this->query('import_export_operation', $tenantId)
            ->where(function (BaseQuery $query) use ($fileKey): void {
                $query->where('result_file_key', $fileKey)->whereOr('error_file_key', $fileKey);
            })
            ->whereIn('status', ['succeeded', 'failed'])
            ->where('retention_until', '>', new Raw('UTC_TIMESTAMP(3)'))->find()?->toArray();
        if ($row === null) {
