...
按逐符号映射同步namespace/use/全名/同namespace隐式引用、Composer/Provider/路由/监听/任务/公开合同、调用者/生成器/测试/构建/接口元数据/模块路径及正式摘要。旧Kernel业务和技术同前缀，禁止整前缀替换。持久化类名/序列化/任务需明确过渡；临时别名有消费者、测试、退出期，不双实现。迁移不重命名表/ID/权限/URL，不重跑已应用SQL。
