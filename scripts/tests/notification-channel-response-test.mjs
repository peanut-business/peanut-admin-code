...
const compiler = require(path.join(root, 'web/node_modules/@vue/compiler-sfc'));
const apiPath = path.join(root, 'web/src/modules/official-notification/api.ts');
const pagePath = path.join(root, 'web/src/modules/official-notification/views/channel/index.vue');
const apiSource = fs.readFileSync(apiPath, 'utf8');
const pageSource = fs.readFileSync(pagePath, 'utf8');
const parsed = compiler.parse(pageSource, { filename: pagePath });
assert.deepEqual(parsed.errors, []);
assert.ok(parsed.descriptor.scriptSetup, 'The actual page must have a script setup block.');
const pageScript = parsed.descriptor.scriptSetup.content;

const valid = () => ({
  sms_default: 'aliyun',
  sms_aliyun: { access_key_id: 'synthetic-id', access_key_secret: '******', sign_name: 'synthetic', status: 1 },
  sms_tencent: { secret_id: '', secret_key: '', sdk_app_id: '', sign_name: '', region: 'ap-guangzhou', status: 0 },
  status: { sms: true },
});
...
function strictCheck(typescript) {
  ...
  const options = {
    ...
    baseUrl: path.join(root, 'web'), paths: { '@/*': ['src/*'] }, types: [],
  };
  ...
  const program = typescript.createProgram([apiPath, virtualPath], options, host);
  const errors = typescript.getPreEmitDiagnostics(program);
  assert.equal(errors.length, 0, typescript.formatDiagnosticsWithColorAndContext(errors, {
...
test('API and page do not bypass the response boundary with assertions or suppression', () => {
  for (const source of [apiSource, pageScript]) {
    assert.doesNotMatch(source, /\bas\s+unknown\s+as\b|\bas\s+any\b|@ts-(?:ignore|nocheck)/);
  }
});
...
for (const provider of ['', 'aliyun', 'tencent']) {
  test('preserves legal provider, flags and redacted secrets: ' + JSON.stringify(provider), async () => {
    const data = valid();
    data.sms_default = provider;
    const response = { code: 20000, msg: '', data, trace: 'synthetic-metadata' };
    ...
    assert.deepEqual(requests, [{ method: 'GET', url: '/adminapi/official.notification.channel.detail' }]);
  });
}
...
for (const [name, change] of [
  ...
  ['missing provider', (d) => { delete d.sms_default; return d; }],
  ['array provider config', (d) => ({ ...d, sms_aliyun: [] })],
  ['missing provider config', (d) => { delete d.sms_tencent; return d; }],
  ['numeric identifier', (d) => { d.sms_aliyun.access_key_id = 7; return d; }],
  ['missing secret', (d) => { delete d.sms_tencent.secret_key; return d; }],
  ['null secret', (d) => { d.sms_aliyun.access_key_secret = null; return d; }],
  ['string enable flag', (d) => { d.sms_aliyun.status = '1'; return d; }],
  ['unknown enable flag', (d) => { d.sms_tencent.status = 2; return d; }],
  ['missing region', (d) => { delete d.sms_tencent.region; return d; }],
  ['array status', (d) => ({ ...d, status: [] })],
  ['string readiness', (d) => ({ ...d, status: { sms: 'false' } })],
]) {
  test('rejects ' + name + ' before the page consumes configuration', async () => {
    const { api } = loadApi({ data: change(valid()) });
    await assert.rejects(api.getNoticeChannelDetail(), /NOTIFICATION_CHANNEL_RESPONSE_INVALID/);
  });
}
...
test('transport failure remains the same failure', async () => {
  ...
  await assert.rejects(api.getNoticeChannelDetail(), (error) => error === failure);
});
...
test('channel saving retains URL, selected section and the redaction sentinel', async () => {
  ...
  assert.deepEqual(JSON.parse(JSON.stringify(requests)), [{
    method: 'POST', url: '/adminapi/official.notification.channel.save',
