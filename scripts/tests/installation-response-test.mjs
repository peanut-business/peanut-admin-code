...
function client(body, failure) {
  ...
  vm.runInNewContext(output, {
    ...
    __meta: { env: {} },
    require(name) {
      assert.equal(name, 'axios', 'The isolated client must not load a shared session/interceptor');
...
const status = () => ({ state: 'uninstalled', mode: 'guided', deployment_mode: 'standalone', preflight: null });
const envelope = (data) => ({ code: 20000, msg: 'ok', data });
const payload = { admin_email: 'admin@example.test', admin_password: 'synthetic', platform_email: 'platform@example.test', platform_password: 'synthetic', official_modules: ['official.article'] };
const plain = (value) => JSON.parse(JSON.stringify(value));

test('source rejects double assertions instead of suppressing the response boundary', () => {
  let invalid = false;
  const visit = (node) => {
    if (ts.isAsExpression(node) && ts.isAsExpression(node.expression)) invalid = true;
...
test('the entire client typechecks strictly against real installed Axios declarations', () => {
  ...
  assert.ok(fs.existsSync(declaration), 'Install the selected web dependencies; do not synthesize Axios types');
  const ambientPath = path.join(root, '.local/tmp/installation-response-ambient.d.ts');
  const ambient = 'interface ImportMeta { readonly env: { readonly VITE_API_BASE_URL?: string }; }';
  const options = { target: ts.ScriptTarget.ES2022, module: ts.ModuleKind.ESNext, moduleResolution: ts.ModuleResolutionKind.Bundler, strict: true, noEmit: true, skipLibCheck: true, esModuleInterop: true, types: [] };
  ...
  const diagnostics = ts.getPreEmitDiagnostics(ts.createProgram([sourcePath, ambientPath], options, host));
  ...
  for (const state of ['uninstalled', 'installed', 'blocked']) {
    for (const mode of ['guided', 'automatic']) {
      for (const deployment_mode of ['standalone', 'multi-tenant']) {
        ...
        assert.deepEqual(calls, [{ method: 'get', args: ['/installapi/status'] }]);
      }
    }
  }
});
...
test('validated preflight checks, module choices, retry flags and health retain their values', async () => {
  const value = {
    ...status(),
    preflight: { status: 'blocked', checks: [{ id: 'database', status: 'failed', code: 'UNAVAILABLE', remediation: 'Configure the connection' }], modules: ['official.file', { key: 'official.article', name: 'Article', selected: true, required: false }], official_modules: [], extension: { preserved: true } },
    official_modules: [{ key: 'official.file', default: true, label: 'Files', description: 'Media' }],
    retryable: false, code: 'ENVIRONMENT_UNAVAILABLE', health: { ready: false },
  };
  assert.deepEqual(plain(await client(envelope(value)).api.getInstallationStatus()), value);
});

test('an omitted preflight remains the documented null default', async () => {
  const value = status();
  delete value.preflight;
  assert.equal((await client(envelope(value)).api.getInstallationStatus()).preflight, null);
});
...
for (const [name, patch] of [
  ...
  ['invalid preflight official modules', { preflight: { official_modules: [{ key: false }] } }],
  ...
  ['string selected flag', { official_modules: [{ key: 'official.file', selected: 'false' }] }],
  ['invalid optional module label', { official_modules: [{ key: 'official.file', label: [] }] }],
  ['string retry flag', { retryable: 'false' }],
  ['numeric error code', { code: 10 }],
  ['array health', { health: [] }],
]) {
  test(`status rejects ${name}`, async () => {
    await assert.rejects(client(envelope({ ...status(), ...patch })).api.getInstallationStatus(), /invalid/i);
  });
}

for (const [name, body] of [['null envelope', null], ['missing data', { code: 20000 }], ['string success code', { code: '20000', data: status() }]]) {
...
test('application errors and transport failures remain failures', async () => {
  await assert.rejects(client({ code: 50000, msg: 'synthetic server failure', data: null }).api.getInstallationStatus(), /synthetic server failure/);
  const failure = new Error('synthetic transport failure');
  await assert.rejects(client(null, failure).api.getInstallationStatus(), (error) => error === failure);
});

test('execute keeps the one-time token and explicit write payload', async () => {
  const value = { state: 'installed', health: { ready: true } };
  const { api, calls } = client(envelope(value));
  const result = await api.executeInstallation(' synthetic-token ', { ...payload, tenant_id: 999 });
  assert.deepEqual(plain(result), value);
  assert.deepEqual(plain(calls), [{ method: 'post', args: ['/installapi/execute', payload, { headers: { Authorization: 'Bearer synthetic-token' } }] }]);
});
...
for (const [name, value] of [['array health', { state: 'installed', health: [] }], ['null health', { state: 'installed', health: null }], ['missing health', { state: 'installed' }], ['wrong state', { state: 'uninstalled', health: {} }]]) {
  test(`execute rejects ${name}`, async () => {
    await assert.rejects(client(envelope(value)).api.executeInstallation('synthetic-token', payload));
  });
}
