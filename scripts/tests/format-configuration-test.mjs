...
test('the formatter version is explicit and matches the installed tool', () => {
  const manifest = JSON.parse(
    readFileSync(resolve(root, 'tools/quality/package.json'), 'utf8')
  );
...
test('contradictory modes fail before any source enumeration or writes', () => {
  assert.throws(
    () => formatSources(['--check', '--write']),
    /FORMAT_MODE_CONFLICT/
  );
  assert.throws(
    () => formatSources(['--write', '--check']),
    /FORMAT_MODE_CONFLICT/
  );
});
...
for (const path of [
  ...
  test(
    'native configuration discovery finds the same policy for ' + path,
    () => {
      assert.deepEqual(
        prettier.resolveConfig.sync(resolve(root, path)),
        policy
      );
    }
  );
...
test('the formatting entry excludes immutable and generated sources', () => {
  for (const path of [
    'scaffold/releases/v1/files/web/main.ts',
    'server/tests/fixtures/broken.ts',
...
test('generated component declarations retain their bytes without excluding owned declarations', () => {
  const options = {
    ignorePath: resolve(root, 'tools/quality/prettier-ignore'),
  };
  const generated = resolve(root, 'web/components.d.ts');
  assert.match(
    readFileSync(generated, 'utf8'),
    /Generated by unplugin-vue-components/
  );
  assert.equal(prettier.getFileInfo.sync(generated, options).ignored, true);
  assert.equal(
    prettier.getFileInfo.sync(resolve(root, 'uniapp/shims-uni.d.ts'), options)
      .ignored,
    false
  );
});
...
for (const [parser, input] of [
  ['typescript', 'export const value={a:"x",b:[1,2]}\r\n'],
  [
    'vue',
  ...
  test(
    'native formatter is idempotent on a synthetic ' + parser + ' fixture',
    () => {
      const formatted = prettier.format(input, { ...policy, parser });
      assert.equal(
        prettier.format(formatted, { ...policy, parser }),
        formatted
      );
