...
test('generated component declarations retain their bytes without excluding owned declarations', () => {
  const options = { ignorePath: resolve(root, 'tools/quality/prettier-ignore') };
  const generated = resolve(root, 'web/components.d.ts');
  assert.match(readFileSync(generated, 'utf8'), /Generated by unplugin-vue-components/);
  assert.equal(prettier.getFileInfo.sync(generated, options).ignored, true);
  assert.equal(prettier.getFileInfo.sync(resolve(root, 'uniapp/shims-uni.d.ts'), options).ignored, false);
});
