...
function layoutGit(string $root, array $arguments): string
{
    ...
    if (!is_resource($process)) throw new RuntimeException('LAYOUT_GIT_UNAVAILABLE');
    $stdout = stream_get_contents($pipes[1]);
    $stderr = stream_get_contents($pipes[2]);
    fclose($pipes[1]); fclose($pipes[2]);
    if (proc_close($process) !== 0) throw new RuntimeException('LAYOUT_GIT_FAILED: ' . trim($stderr));
    return $stdout;
}
...
{
    if (preg_match('/^\s*(?:function|const)\s+/', $source)) return [];
    $prefix = '';
    ...
    foreach (explode(',', $source) as $item) {
        $item = trim($item);
        if (preg_match('/^(?:function|const)\s+/', $item)) continue;
        $item = preg_split('/\s+as\s+/i', $item)[0];
        $name = ltrim(trim($prefix . $item), '\\');
        if (preg_match('/^[A-Za-z_][A-Za-z0-9_]*(?:\\\\[A-Za-z_][A-Za-z0-9_]*)*$/D', $name)) $names[] = $name;
    }
    return $names;
}
...
function layoutSymbols(string $source): array
{
    ...
    $namespace = ''; $depth = 0; $namespaceDepth = 0; $declarations = []; $references = [];
    $skip = [T_WHITESPACE, T_COMMENT, T_DOC_COMMENT];
    for ($i = 0, $count = count($tokens); $i < $count; $i++) {
        $token = $tokens[$i];
        if (is_string($token)) {
            if ($token === '{') $depth++;
            if ($token === '}') { $depth--; if ($depth < $namespaceDepth) { $namespace = ''; $namespaceDepth = $depth; } }
            continue;
        }
        ...
        if ($kind === T_NAMESPACE) {
            ...
            while (++$i < $count) {
                $next = $tokens[$i];
                if ($next === ';') { $namespaceDepth = $depth; break; }
                if ($next === '{') { $depth++; $namespaceDepth = $depth; break; }
                if (is_array($next) && !in_array($next[0], $skip, true)) $namespace .= $next[1];
            }
            $namespace = trim($namespace, '\\');
        } elseif (in_array($kind, [T_CLASS, T_INTERFACE, T_TRAIT, T_ENUM], true)) {
            $j = $i + 1;
            while ($j < $count && is_array($tokens[$j]) && in_array($tokens[$j][0], $skip, true)) $j++;
            ...
            $j = $i + 1;
            while ($j < $count && is_array($tokens[$j]) && in_array($tokens[$j][0], $skip, true)) $j++;
            if (($tokens[$j] ?? null) === '(') continue; // Closure capture, not a namespace import.
            $use = '';
            while ($j < $count && $tokens[$j] !== ';') {
                $part = $tokens[$j++];
                if (is_string($part)) $use .= $part;
                elseif (!in_array($part[0], [T_COMMENT, T_DOC_COMMENT], true)) $use .= $part[1];
            }
            foreach (layoutImports($use) as $name) $references[] = compact('name', 'line');
            $i = $j;
...
function checkSourceLayout(array $roots): array
{
    ...
    foreach ($roots as $input) {
        ...
        if (isset($repositories[$root])) throw new InvalidArgumentException('LAYOUT_DUPLICATE_ROOT');
        ...
        $casePaths = []; $sources = []; $mappings = []; $history = 0;
        foreach ($paths as $path) {
            ...
            if (layoutHistorical($path)) { $history++; continue; }
            if (array_intersect($parts, ['vendor', 'node_modules', '.local', '.git']) !== []) continue;
            if (!str_ends_with($path, '.php') && basename($path) !== 'composer.json') continue;
            $absolute = $root . '/' . $path;
            if (is_link($absolute) || !is_file($absolute)) {
                $findings[] = ['code' => 'SOURCE_NOT_REGULAR', 'root' => $root, 'path' => $path]; continue;
            }
            $source = file_get_contents($absolute);
            if (!is_string($source)) throw new RuntimeException('LAYOUT_SOURCE_UNREADABLE');
            ...
            $sources[$path] = $parsed;
            foreach ($parsed['declarations'] as $name => $line) $symbols[strtolower($name)][$name][] = [$root, $path, $line];
        }
        foreach ($casePaths as $variants) {
            if (count($variants) > 1) $findings[] = ['code' => 'GIT_PATH_CASE_COLLISION', 'root' => $root, 'paths' => array_keys($variants)];
        }
        $checked = 0; $explicitlyLoaded = 0;
        foreach ($sources as $path => $source) {
            ...
            foreach ($mappings as $mapping) {
                if (str_starts_with($path, $mapping['base'] . '/')) $insideMappedTree = true;
            }
            if (!$insideMappedTree) { $explicitlyLoaded += count($source['declarations']); continue; }
            foreach ($source['declarations'] as $name => $line) {
                $expected = []; $longest = -1;
                foreach ($mappings as $mapping) {
                    if (!str_starts_with($name, $mapping['namespace'])) continue;
                    $length = strlen($mapping['namespace']);
                    if ($length < $longest) continue;
                    if ($length > $longest) { $expected = []; $longest = $length; }
                    $expected[] = $mapping['base'] . '/' . str_replace('\\', '/', substr($name, $length)) . '.php';
                }
                if ($expected === []) continue; // Explicit classmap / non-PSR file: not guessed into PSR-4.
                $checked++;
                if (!in_array($path, $expected, true)) $findings[] = ['code' => 'PSR4_DECLARATION_PATH_MISMATCH', 'root' => $root, 'path' => $path, 'line' => $line, 'type' => $name, 'expected' => array_values(array_unique($expected))];
            }
        }
...
if (realpath($_SERVER['SCRIPT_FILENAME'] ?? '') === __FILE__) {
    try {
        ...
        foreach (array_slice($argv, 1) as $argument) {
            if (!str_starts_with($argument, '--root=') || strlen($argument) === 7) throw new InvalidArgumentException('Usage: php scripts/check-source-layout.php --root=<Git root> [--root=<other root>]');
            $roots[] = substr($argument, 7);
        }
        if ($roots === []) $roots[] = dirname(__DIR__);
