...
export function isPcPrivateRoute(path: string): boolean {
  return pcPrivateRouteRoots.some(root => path === root || path.startsWith(`${root}/`))
}

interface PrivateRouteRule {
  ssr: false
  prerender: false
  cache: false
...
/**
 * hybrid：公开内容 SSR，用户区域 CSR；spa：全部客户端渲染。
...
export function createPcRenderingOptions(value: string | undefined) {
  ...
  const routeRules: Record<string, PrivateRouteRule> = {}
