...
test('Nuxt SSR keeps alternating Tenant HTML and request context isolated', async (context) => {
  const nitroSourceDirectory = new URL(
    '../../.output/server/',
    import.meta.url
  );
  const testNitroDirectory = new URL(
    `../../.output/ssr-test-${process.pid}/`,
    import.meta.url
  );
  const testNitroPath = new URL('chunks/nitro/nitro.mjs', testNitroDirectory);
  const recordPath = new URL(
    `../../.output/ssr-upstream-${process.pid}.jsonl`,
    import.meta.url
  );
  context.after(() => {
  ...
  const localSource = `${source.slice(
    0,
    start
  ...
  const nitroModule = await import(
    `${pathToFileURL(testNitroPath.pathname).href}?test=${Date.now()}`
  );
  ...
  for (const request of upstreamRequests) {
    const tenant = request.host.startsWith('tenant-a.')
      ? 'tenant-a'
      : 'tenant-b';
    ...
    assert.equal(
      request.cookie,
      `tenant_session=${tenant}; token=${tenant}-token`
    );
